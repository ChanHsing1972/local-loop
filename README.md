...
启动界面会显示当前模型、工作区和审批模式。直接输入编程任务即可；终端会流式显示正文，并把“读取哪个文件、搜索什么文本、写入多少字节、运行什么命令、退出码和输出摘要”等具体事实逐项展示。输入 `/` 会自动在输入框下方展开全部可执行命令及说明，可用上下键选择并按 Tab 补全；继续输入前缀会即时筛选。模型完成一轮后程序不会退出，可以继续追问，例如要求补充测试、解释设计或重新检查结果。输入历史保存在被 Git 忽略的 `.localloop/input_history` 中。
...
提交前运行 `python scripts/release_check.py` 检查受控文件与完整 Git 历史，确认所有 Author 和 Committer 都是 `Liu Yuxuan <231880433@smail.nju.edu.cn>`；再运行 `python scripts/release_check.py --online`，从未登录视角确认仓库已经公开。录制时不要展示 API 值、`.env`、无关通知或本机敏感路径。完整人工步骤见 `docs/最终验收清单.md`。
