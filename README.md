...
提交前运行 `python scripts/release_check.py` 检查受控文件与完整 Git 历史，再运行 `python scripts/release_check.py --online`，从未登录视角确认仓库已经公开且账号主页没有姓名、学校、单位、地区或个人链接等身份线索。脚本无法判断头像和用户名本身是否可识别，这两项仍须人工检查。终端提示符、浏览器头像、菜单栏、绝对路径和通知也不得进入演示视频。完整人工步骤见 `docs/最终验收清单.md`。
